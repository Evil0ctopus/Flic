...
void WebUiEngine::handleRoot() {
    ...
    html += "let faceCatalog={styles:[]};function refreshFaceAnimations(){if(!faceStyle||!facePreviewAnim)return;const selected=String(faceStyle.value||'default');const style=(faceCatalog.styles||[]).find(s=>String(s.name||'')===selected);facePreviewAnim.innerHTML='';const items=(style&&Array.isArray(style.animations))?style.animations:[{name:'idle'}];for(const it of items){const n=String(it.name||'idle');const o=document.createElement('option');o.value=n;o.textContent=n;facePreviewAnim.appendChild(o);}if(facePreviewAnim.options.length===0){const o=document.createElement('option');o.value='idle';o.textContent='idle';facePreviewAnim.appendChild(o);}}";
    ...
    html += "async function saveFaceSettings(){const payload={active_style:String(faceStyle.value||'default'),blink_speed:Number(faceBlink.value),idle_enabled:!!faceIdleEnabled.checked,glow_intensity:Number(faceGlow.value),eye_color:String(faceEyeColor.value||'#AEE6FF'),ai_can_modify:!!faceAiModify.checked,ai_can_create:!!faceAiCreate.checked};const r=await api('/api/face/settings',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload)});log('face settings => '+JSON.stringify(r));refresh();}";
